    ...
    fun slimImage(
        ...
        project?.asyncTask(hintText = "正在压缩", runAction = {
            ...
            if (outputSameFile){//针对右键选定图片情况，直接压缩当前目录选中图片，输出目录包括文件也是原来的
                ...
                inputFiles.forEach { if (model.filePrefix.isBlank()) { Tinify.fromFile(it.absolutePath).toFile("${model.outputDir}/${it.name}") } else { Tinify.fromFile(it.absolutePath).toFile("${model.outputDir}/${model.filePrefix}_${it.name}") } }
            }
        }, successAction = {
